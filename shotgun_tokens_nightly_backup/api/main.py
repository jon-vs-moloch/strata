...
from shotgun_tokens.storage.models import TaskModel, TaskType
...
app = FastAPI(title="Shotgun Tokens API", lifespan=lifespan)
...
async def post_chat(payload: Dict[str, Any], storage: StorageManager = Depends(get_storage)):
    ...
    messages.append({
        "role": "system",
        "content": """You are the Shotgun Tokens Orchestrator. You are a STRICT ROUTING AGENT. 
