...
- operator-facing control is a product requirement, not a debug extra: if Strata has a meaningful subsystem or capability, the operator should normally be able to inspect and operate it from the UI unless that surface is deliberately withheld for policy, safety, or maturity reasons
- that control should remain optional in practice: Strata should work out of the box on day 1 without requiring the operator to become its mechanic
- Strata should feel like a manual transmission with strong autopilot: every gear should be available, but the system should still be able to handle the ordinary driving itself
...
- tools should be allowed to carry recommended or mandatory Procedures for their safe/effective use, and Procedures should be able to specify one or more tools; the system should model and watch for circular tool <-> Procedure loops rather than pretending they do not exist
...
- this unification direction should apply broadly, not just to `Audit`: as much of Strata as possible should eventually be made from normal Strata primitives such as Procedures, tools, Knowledge, policies, Kits, and auditable runtime artifacts rather than opaque special-purpose internals
...
- communications should become a normal substrate rather than a bolt-on integration: Strata should eventually operate across chat, email, Slack, Discord, voice, and similar lanes while preserving provenance, safety checks, and lane-aware routing
- Strata should accumulate a real notion of schedules and time commitments: it should learn the operator's schedule, be able to maintain its own schedule where needed, and later integrate with external calendars without requiring those integrations to exist first
- common day-1 utility surfaces matter: calculator, image generation, website publishing, and other basic tools/apps should be treated as core capability work rather than niche extras
- conversational presence is part of capability: if the system is thinking, working, or waiting, it should be noticeably communicative enough that the operator does not experience the machine as mute or inert
